...
> 🔒 Security Notice: This Skill only outputs text content. It does not execute system commands or call external APIs.
>
> **⚠️ File Read Authorization Rule (Mandatory · Privacy Request)**:
> - Reading local files **is a privacy request**. A confirmation prompt must pop up listing the file/folder paths to be read; reading can only proceed after explicit user consent.
> - Before user confirmation, **forbidden** to call any file reading tool (Read / Glob / Grep / cat/ls in Bash, etc.).
> - **User declines → show a final warning first** (⚠️ "If you decline authorization now, the handover pack cannot be generated properly. Confirm termination?"), and **only terminate after the user confirms termination a second time** (A). If the user selects B (go back), return to the authorization prompt. This Skill depends on file reading to generate the handover pack; without authorization it cannot complete, and **downgrading to manual input is not allowed**.
>
> **Exception**: In Step F "Generate Memory Pack", the `Write` tool may be called to create 10 Markdown deliverable files on the Desktop (this is the Skill's core deliverable, no extra confirmation needed).
...
→ Step B5: File read authorization (A/B/C, decline=terminate) → ⏸️ STOP → User chooses
→ Step B6: Summary confirmation → ⏸️ STOP → User verifies
→ Step C: Browse & analyze (with C0 read confirmation, progress bar) → ⏸️ STOP → User confirms
...
### B5. File Read Authorization Confirmation (must ask before browsing, mandatory prompt)

```
⚠️ 🔐 File Read Authorization Request (Privacy Request)

Reading local files is a privacy request and requires your explicit authorization.

About to read local files at the following paths:

📁 Project root: [project path]
📁 Browse scope: project root and its subdirectories
📋 Read content: file structure, README, config files, source files, etc.

Please confirm:
A. ✅ Authorize reading (agree to read files at the above paths, continue)
B. ❌ Decline reading (I do not authorize reading)
C. ⚠️ Partial authorization (specify allowed / forbidden paths)

⏸️ Please choose (A/B/C). No local files will be read before authorization.
```

**⏸️ Stop after output. Before user confirmation, forbidden to perform any file reading operation.**

**If B is selected (Decline reading)**:
```
⚠️ Warning: If you decline authorization at this point, this Skill will not be able to read local project files,
and the handover pack cannot be generated properly.

Confirm whether to terminate generation?

A. ✅ Confirm termination (exit task, handover pack will not be generated)
B. ❌ Go back, re-choose (I want to authorize reading)
```
**⏸️ Stop after output.**

**If user selects A (Confirm termination)**:
```
🛑 Task terminated

You have confirmed declining file read authorization. This Skill depends on reading local project files to generate the handover pack,
and cannot continue without authorization.

To restart, please enter a trigger word again (e.g. "IRY") to launch the flow.

✋ IRY flow exited.
```
**⏸️ Stop after output, terminate the flow, do not enter any subsequent Step.**

**If user selects B (Go back)**: Return to B5 options, wait for user to choose A/B/C again.

---

**If C is selected**:
```
Please specify the authorization scope:

1. Allowed paths (separate multiple with commas):
   Example: ./src, ./README.md, ./package.json

2. Forbidden paths (separate multiple with commas):
   Example: ./config, ./env, ./data/private
```

**⏸️ Stop after output. AI only reads allowed paths and skips forbidden paths during browsing.**
...
| Project Name | [XXX] |
| Project Path | [XXX] |
| File Read Authorization | [Authorized / Declined / Partial: XXX] |
...
### C0. Read Operation Second Confirmation (mandatory prompt before reading)

**Before AI calls any file reading tool, it must output the following confirmation box:**

```
🔐 About to perform file read operation

This browse will read the following paths:
- [project path] (root)
- [project path]/README.md
- [project path]/*.json (config files)
- [project path]/src/ or other source directories
- [other paths to read]

Estimated files to read: [N]
Authorization status: [Authorized / Partial]

Confirm to read?
A. ✅ Confirm, start reading
B. ❌ Cancel (I do not authorize reading)

⏸️ Please choose (A/B).
```

**⏸️ Stop after output. AI may only call Read / Glob / Grep tools after user selects A.**

**If B is selected (Cancel)**:
```
⚠️ Warning: If you cancel at this point, this Skill will not be able to read local project files,
and the handover pack cannot be generated properly.
...
**If user selects B (Go back)**: Return to C0 options, wait for user to choose A/B/C again.
...
```
✅ Received your补充 information: [User's description]

Re-scanning the project based on the补充 requirements...
[Rescan progress]
...
### ⚠️ Critical Iron Rule (Violation = Task Failure)

> **AI MUST use the `Write` tool to actually write files. Text progress output alone is strictly forbidden.**
> "Progress bar" ≠ "real write". Each file must be created by one `Write` tool call.
> **After all 10 files are written, use `ls` or `find` to verify they actually exist**, then enter Step G.
> If only text is output without real files, **this is judged as failure** and Step F must be re-executed.
...
**Directory creation**:
- AI must first use the `mkdir -p` tool to create the directory
- Then use the `Write` tool to write files one by one
...
- **Removed all quiz questions** → "Browse → Checklist → Verify → Rescan" closed loop (no questions, no exams)
